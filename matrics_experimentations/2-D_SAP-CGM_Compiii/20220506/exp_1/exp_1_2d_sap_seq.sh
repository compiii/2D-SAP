#!/bin/bash
#SBATCH --job-name=2-D SAP
#SBATCH --partition=long            # submission queue (normal or bigmem)
#SBATCH --time=4:00:00            # 1-1 means one day and one hour
...
#SBATCH --mem-per-cpu=64G          # Memory per processor
...
for n in `cat input/parameters/param-n.data` 
    do
        mpirun -np 1 ./2D_SAP.run 0 $n $n $n $n
    done
