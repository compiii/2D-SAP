...
int main(int argc, char *argv[])
{
    ...
    if ((algorithm == 1 || run_all_algorithms))
    {
        ...
        if constexpr (1)
        {
            ...
            if (output)
            {
                output << algorithm << "," << 0 << "," << rank << "," << max_process << "," << nb_str_1 << "," << size_str_1 << "," << nb_str_2 << "," << size_str_2 << "," << total_duration << "," << pre_comm_time << "," << preprocessing_time << "," << main_comm_time << "," << computation_time << "," << score << "," << std::endl;
                output.close();
            }
        }
    }
